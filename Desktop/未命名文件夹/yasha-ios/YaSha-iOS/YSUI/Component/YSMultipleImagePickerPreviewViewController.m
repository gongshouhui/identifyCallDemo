//
//  YSMultipleImagePickerPreviewViewController.m
//  YaSha-iOS
//
...
@interface YSMultipleImagePickerPreviewViewController ()

@end

@implementation YSMultipleImagePickerPreviewViewController{
    ...
    if (asset.assetType == QMUIAssetTypeAudio || asset.assetType == QMUIAssetTypeVideo) {
        ...
        if (_originImageCheckboxButton.selected) {
            [asset assetSize:^(long long size) {
                [_originImageCheckboxButton setTitle:[NSString stringWithFormat:@"原图(%@)", [YSUIHelper humanReadableFileSize:size]] forState:UIControlStateNormal];
                [_originImageCheckboxButton sizeToFit];
                [_bottomToolBarView setNeedsLayout];
            }];
        }
