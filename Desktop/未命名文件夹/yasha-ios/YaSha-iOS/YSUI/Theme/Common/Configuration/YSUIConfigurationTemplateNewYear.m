//
//  YSUIConfigurationTemplateNewYear.m
//  YaSha-iOS
//
//  Created by 方鹏俊 on 2018/1/26.
...
@implementation YSUIConfigurationTemplateNewYear
...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarBackgroundImage = [YSUIHelper navigationBarBackgroundImageWithThemeColor:self.themeTintColor]; // NavBarBackgroundImage : UINavigationBar 的背景图
...
- (NSArray *)themeTabBarItemImage {
    ...
    return @[[self getOriginImageWithName:@"message(newyear)"],
             [self getOriginImageWithName:@"contact(newyear)"],
             [self getOriginImageWithName:@"application(newyear)"],
             [self getOriginImageWithName:@"mine(newyear)"],
             ];
}
- (NSArray *)themeTabBarItemImageSelected {
    return @[[self getOriginImageWithName:@"message_selected(newyear)"],
             [self getOriginImageWithName:@"contact_selected(newyear)"],
             [self getOriginImageWithName:@"application_selected(newyear)"],
             [self getOriginImageWithName:@"mine_selected(newyear)"],
             ];
}

/**tabbar item正常状态下的颜色*/
- (UIColor *)themeTabBarItemNormalColor {
    return [UIColor colorWithRed:0.42 green:0.42 blue:0.42 alpha:1.00];
}
/**tabbar item选中状态下的颜色*/
...
- (UIImage *)themeTabBarBackgroundImage {
    return [UIImage imageNamed:@"tabBarBackgroundImage(newyear)"];
    
}
- (UIImage *)themeLogoImage {
    return [UIImage imageNamed:@"logo(newyear)"];
}

- (UIImage *)themeContactBackgroundImage {
    return [UIImage imageNamed:@"contact_bg(newyear)"];
    
}

- (UIImage *)themeMineBackgroundImage {
    return [UIImage imageNamed:@"mine_bg(newyear)"];
}
...
- (UIImage *)themeWorkItemImageWithItemType:(WorkbenchItemType)itemType {
    if (itemType == WorkbenchItemNeedToDo) {
        return [UIImage imageNamed:@"待办"];
    }
    if (itemType == WorkbenchItemCalendar) {
        return [UIImage imageNamed:@"日程"];
    }
    if (itemType == WorkbenchItemNewsBulletin) {
        return [UIImage imageNamed:@"公文"];
    }
    if (itemType == WorkbenchItemHR) {
        return [UIImage imageNamed:@"workService"];
    }
    if (itemType == WorkbenchItemAssets) {
        return [UIImage imageNamed:@"固定资产"];
    }
    if (itemType == WorkbenchItemRepair) {
        return [UIImage imageNamed:@"IT运维"];
    }
    if (itemType == WorkbenchItemPMSZS) {
        return [UIImage imageNamed:@"ic_app_pm_zhuang"];
    }
    if (itemType == WorkbenchItemPMSMQ) {
        return [UIImage imageNamed:@"ic_app_pm_mu"];
    }
    if (itemType == WorkbenchItemSupply) {
        return [UIImage imageNamed:@"供应链"];
    }
    if (itemType == WorkbenchItemEMS) {
        return [UIImage imageNamed:@"EMS"];
    }
    if (itemType == WorkbenchItemRecharge) {
        return [UIImage imageNamed:@"一卡通充值"];
    }
    if (itemType == WorkbenchItemVedioMetting) {
        return [UIImage imageNamed:@"ico-云视频"];
    }
    if (itemType == WorkbenchItemAdd) {
        return [UIImage imageNamed:@"添加应用"];
    }
    return nil;
}
