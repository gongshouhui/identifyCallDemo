...
#define UIColorTheme0 UIColorMake(238, 133, 193)
...
extern NSString *const YSSelectedThemeClassName;

#define CodeFontMake(_pointSize) [UIFont fontWithName:@"Menlo" size:_pointSize]
#define CodeAttributes(_fontSize) @{NSFontAttributeName: CodeFontMake(_fontSize), NSForegroundColorAttributeName: [YSThemeManager sharedInstance].currentTheme.themeCodeColor}

// QMUIButton 系列 Demo 里的一行高度
extern const CGFloat YSButtonSpacingHeight;

@interface YSCommonUI : NSObject

+ (void)renderGlobalAppearances;

@end

@interface YSCommonUI (ThemeColor)

+ (UIColor *)randomThemeColor;

@end

@interface YSCommonUI (Layer)
