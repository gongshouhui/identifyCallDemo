...
// 记录
#import "YSHRMRecordViewController.h"
...
@interface YSHRMRecordViewController ()<FSCalendarDataSource, FSCalendarDelegate, UITableViewDelegate, UITableViewDataSource, QMUINavigationTitleViewDelegate, QMUINavigationControllerDelegate>
...
@property (nonatomic, assign) BOOL isClickdeShow;
...
@implementation YSHRMRecordViewController
...
- (void)viewDidLoad {
    ...
    // 点击汇总页面的 月度统计 跳转过来
    [[[NSNotificationCenter defaultCenter] rac_addObserverForName:[NSString stringWithFormat:@"YSHRTMonth%ldVCJumpDetail", self.navigationController.viewControllers.count] object:nil] subscribeNext:^(NSNotification * _Nullable x) {
        @strongify(self);
        [self.monthCalendarView.calendar deselectDate:newdate];
        NSDateFormatter *formatterNotice = [[NSDateFormatter alloc] init];
        [formatterNotice setDateFormat:@"yyyy-MM"];
        [formatterNotice setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:8]];
        NSDate *newdate = [formatterNotice dateFromString:[x.userInfo objectForKey:@"time"]];
        self.noticeTimeStr = [x.userInfo objectForKey:@"time"];
        self.searchDateStr = [x.userInfo objectForKey:@"time"];
        self.isNotice = YES;
        self.monthCalendarView.calendar.currentPage = newdate;
...
- (void)doNetworking {
    ...
    [YSNetManager ys_cancelRequestWithURL:[NSString stringWithFormat:@"%@%@", YSDomain, getDepartmentPunchRecords]];
    ...
    [paramDic setObject:self.noticeTimeStr ? self.noticeTimeStr : self.dateStr forKey:@"dateStr"];
