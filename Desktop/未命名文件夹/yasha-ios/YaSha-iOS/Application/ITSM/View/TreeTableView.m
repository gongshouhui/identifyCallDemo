...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_treeTableCellDelegate && [_treeTableCellDelegate respondsToSelector:@selector(cellClick:name:)]) {
        for (int i=0; i<_tempData.count; i++) {
            Node *node = [_tempData objectAtIndex:i];
            DLog(@"======%@------%@",parentNode.parentId,node.nodeId);
            if (parentNode.parentId == node.nodeId) {
                name1 = [NSString stringWithFormat:@"%@/%@/%@",name1,node.name,parentNode.name];
                Node *model = node;
                for (int i=0; i<_tempData.count; i++) {
                    Node *node = [_tempData objectAtIndex:i];
                    if (model.parentId == node.nodeId) {
                        name1 = [NSString stringWithFormat:@"%@%@",node.name,name1];
                    }
                }
            }
        }
        DLog(@"=======%@",name1);
        [_treeTableCellDelegate cellClick:parentNode name:name1];
    }
