//
//  YSPMSMQPlanDetailsViewController.m
//  YaSha-iOS
//
...
@interface YSPMSMQPlanDetailsViewController ()
@property (nonatomic, strong) NSMutableArray *dataArray;
@property (nonatomic, strong) NSMutableArray *photoArray;
@property (nonatomic, strong) NSMutableDictionary *contentOffsetDictionary;
@property (nonatomic, strong) YSPMSMQPlanHeaderView *PMSPlanHeaderView;
...
@property (nonatomic, assign) CGSize strSize;


@end

@implementation YSPMSMQPlanDetailsViewController
...
- (void)doNetworking {
    ...
    [YSNetManager ys_request_GETWithUrlString:[NSString stringWithFormat:@"%@%@/%@",YSDomain,getPlanTaskDetailMQ,self.code] isNeedCache:NO parameters:nil successBlock:^(id response) {
        DLog(@"=======%@",response);
        [QMUITips hideAllToastInView:self.view animated:YES];
        self.strSize = [response[@"data"][@"taskNotes"] boundingRectWithSize:CGSizeMake(kSCREEN_WIDTH-40, 400) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:18]} context:nil].size;
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return (270+self.strSize.height)*kHeightScale;
}
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, kSCREEN_WIDTH, (255+self.strSize.height)*kHeightScale)];
    backView.backgroundColor = [UIColor whiteColor];
    
    self.PMSPlanHeaderView = [[YSPMSMQPlanHeaderView alloc]initWithFrame:CGRectMake(0, 0, kSCREEN_WIDTH, (255+self.strSize.height)*kHeightScale)];
