//
//  YSPMSMQPlanStartsViewController.m
//  YaSha-iOS
//
...
@interface YSPMSMQPlanStartsViewController ()<SWTableViewCellDelegate,UISearchBarDelegate,UISearchControllerDelegate>
...
@implementation YSPMSMQPlanStartsViewController
...
- (void)doNetworking {
    ...
    [YSNetManager ys_request_GETWithUrlString:[NSString stringWithFormat:@"%@%@/%@/%zd/%zd",YSDomain, getPlanTaskListMQ, self.type,self.taskType, self.pageNumber] isNeedCache:NO parameters:payload successBlock:^(id response) {
        DLog(@"==========%@",response);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    YSPMSMQPlanDetailsViewController *PMSPlanDetailsViewController = [[YSPMSMQPlanDetailsViewController alloc]initWithStyle:UITableViewStyleGrouped];
    YSPMSPlanListModel *model = self.dataSourceArray[indexPath.row];
...
-(void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerRightUtilityButtonWithIndex:(NSInteger)index {
    ...
    YSPMSMQPlanAddPhotoViewController *PMSPlanAddPhotoViewController = [[YSPMSMQPlanAddPhotoViewController alloc]init];
    ...
    if ([model.taskCategory isEqual:@"gjkzd"]) {
        ...
        if ([model.taskStatus isEqual:@"10"]) {//开工
            PMSPlanAddPhotoViewController.planType = YSPMSPlanTypeStarts;
            PMSPlanAddPhotoViewController.type = @"1";
