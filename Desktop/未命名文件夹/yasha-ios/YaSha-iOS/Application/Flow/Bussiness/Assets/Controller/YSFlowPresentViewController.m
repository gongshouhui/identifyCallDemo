//
//  YSFlowPresentViewController.m
//  YaSha-iOS
//
//  Created by YaSha_Tom on 2017/11/15.
//

#import "YSFlowPresentViewController.h"
#import "YSFlowExpressFormModel.h"
#import "YSFlowAssetsApplyFormModel.h"

@interface YSFlowPresentViewController ()
...
@implementation YSFlowPresentViewController

- (NSMutableArray *)titleArray {
    if (!_titleArray) {
        _titleArray = [NSMutableArray array];
        [_titleArray addObjectsFromArray:@[@{@"申请信息": @[@"申请单号", @"使用人", @"使用岗位", @"使用部门", @"使用公司", @"项目名称",@"项目经理",@"项目属性",@"申请原因"]}]];
    }
    return _titleArray;
}
    ...
    [YSNetManager ys_request_GETWithUrlString:urlString isNeedCache:NO parameters:nil successBlock:^(id response) {
        ...
        if ([response[@"code"] intValue] == 1) {
            ...
            for (int i = 0; i < self.flowAssetsApplyFormListModel.applyInfos
                ...
                [self.titleArray addObjectsFromArray:@[@{i == 0 ? @"明细信息" : @"" : i == 0 ? @[@"金额合计(元)", @"物品名称", @"数量", @"参考价(元)", @"金额合计(元)"]:@[ @"物品名称", @"数量", @"参考价(元)", @"金额合计(元)"]}]];
