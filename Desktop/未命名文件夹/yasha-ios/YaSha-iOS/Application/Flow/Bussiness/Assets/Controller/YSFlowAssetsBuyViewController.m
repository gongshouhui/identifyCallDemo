//
//  YSFlowAssetsBuyViewController.m
//  YaSha-iOS
//
//  Created by YaSha_Tom on 2017/11/15.
//

#import "YSFlowAssetsBuyViewController.h"
#import "YSFlowExpressFormModel.h"
#import "YSFlowAssetsApplyFormModel.h"

@interface YSFlowAssetsBuyViewController ()
...
@implementation YSFlowAssetsBuyViewController
    ...
    [YSNetManager ys_request_GETWithUrlString:urlString isNeedCache:NO parameters:nil successBlock:^(id response) {
        ...
        if ([response[@"code"] intValue] == 1) {
            ...
            for (int i = 0; i < self.flowAssetsApplyFormListModel.applyInfos
                ...
                [self.titleArray addObjectsFromArray:@[@{i == 0 ? @"明细信息" : @"" : i == 0 ? @[@"金额合计(元)", @"物品名称", @"规格型号",@"数量", @"参考价(元)",@"金额(元)",@"税额(元)", @"价税合计(元)",@"行备注"]:@[ @"物品名称", @"规格型号",@"数量", @"参考价(元)",@"金额(元)",@"税额(元)", @"价税合计(元)",@"行备注"]}]];
                [self.flowDataSourceArray addObjectsFromArray: i == 0? @[@[self.flowAssetsApplyFormListModel.amount,applyInfosModel.goodsName,
                                                                           ...
                                                                           [NSString stringWithFormat:@"%.2f",[applyInfosModel.refPrice floatValue]*[applyInfosModel.applyNumber floatValue]],[NSString stringWithFormat:@"%.2f",[applyInfosModel.totalRate floatValue]],
                                                                            [NSString stringWithFormat:@"%.2f",[applyInfosModel.totalNum floatValue]],applyInfosModel.remark
                                                                           ]]:@[@[applyInfosModel.goodsName,
                                                                                  applyInfosModel.proModel,
                                                                                  [NSString stringWithFormat:@"%@%@",applyInfosModel.applyNumber, applyInfosModel.buyUnit],
                                                                                  applyInfosModel.refPrice,
                                                                                  [NSString stringWithFormat:@"%.2f",[applyInfosModel.refPrice floatValue]*[applyInfosModel.applyNumber floatValue]],
                                                                                  [NSString stringWithFormat:@"%.2f",[applyInfosModel.totalRate floatValue]],
                                                                                  [NSString stringWithFormat:@"%.2f",[applyInfosModel.totalNum floatValue]],
                                                                                  applyInfosModel.remark
                                                                                  
                                                                                  ]]];
                NSLog(@"-----%@",applyInfosModel.totalRate);
            }
            
            NSLog(@"======%@",self.flowDataSourceArray);
