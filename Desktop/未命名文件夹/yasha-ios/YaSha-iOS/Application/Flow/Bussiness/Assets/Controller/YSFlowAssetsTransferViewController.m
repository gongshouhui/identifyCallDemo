//
//  YSFlowAssetsTransferViewController.m
//  YaSha-iOS
//
//  Created by YaSha_Tom on 2017/11/15.
//

#import "YSFlowAssetsTransferViewController.h"
#import "YSFlowExpressFormModel.h"
#import "YSFlowAssetsApplyFormModel.h"

@interface YSFlowAssetsTransferViewController ()
...
@implementation YSFlowAssetsTransferViewController

- (NSMutableArray *)titleArray {
    if (!_titleArray) {
        _titleArray = [NSMutableArray array];
        [_titleArray addObjectsFromArray:@[@{@"申请信息": @[@"申请单号", @"接受人", @"接受岗位", @"接受部门", @"接受公司", @"接受项目",@"项目经理",@"项目属性",@"转移说明"]}]];
    }
    return _titleArray;
}
    ...
    [YSNetManager ys_request_GETWithUrlString:urlString isNeedCache:NO parameters:nil successBlock:^(id response) {
        ...
        if ([response[@"code"] intValue] == 1) {
            ...
            [self.flowDataSourceArray addObjectsFromArray:@[@[self.flowAssetsApplyFormListModel.applyNo,
                                                              ...
                                                              self.flowAssetsApplyFormListModel.receiveJobStation,
                                                              self.flowAssetsApplyFormListModel.receiveDept,
                                                              self.flowAssetsApplyFormListModel.receiveCompany,
                                                              self.flowAssetsApplyFormListModel.ownProject,
                                                              self.flowAssetsApplyFormListModel.managerName,
                                                              self.flowAssetsApplyFormListModel.proNature,
                                                              self.flowAssetsApplyFormListModel.receiveMsg
                                                              ]]];
            for (int i = 0; i < self.flowAssetsApplyFormListModel.accounts
                ...
                [self.titleArray addObjectsFromArray:@[@{i == 0 ? @"明细信息" : @"" : @[@"资产名称", @"规格型号", @"物品说明"]}]];
                [self.flowDataSourceArray addObjectsFromArray:@[@[applyInfosModel.goodsName,
