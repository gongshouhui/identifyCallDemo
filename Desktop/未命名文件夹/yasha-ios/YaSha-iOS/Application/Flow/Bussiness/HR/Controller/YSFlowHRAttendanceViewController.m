//
//  YSFlowHRAttendanceViewController.m
//  YaSha-iOS
//
...
#import "YSFlowHRAttendanceViewController.h"
#import "YSFlowHRInfoViewController.h"
#import "YSFlowAttachmentViewController.h"

@interface YSFlowHRAttendanceViewController ()
...
@implementation YSFlowHRAttendanceViewController
...
- (void)doNetworking {
    NSString *urlString;
    if([self.cellModel.processDefinitionKey isEqual:@"cshr_attendance"]){
        //考勤补录
        urlString = [NSString stringWithFormat:@"%@%@/%@/%@",YSDomain, approveCheckWorkApi, self.cellModel.businessKey,self.cellModel.processInstanceId];
    }else if([self.cellModel.processDefinitionKey isEqual:@"cshr_evection"]){
        urlString = [NSString stringWithFormat:@"%@%@/%@/%@",YSDomain, approveBusinessApi, self.cellModel.businessKey,self.cellModel.processInstanceId];
    }
    DLog(@"=========%@",urlString);
...
- (void)setUpData {
    ...
    [self.expensePersonArr addObject:@{@"补录日期":self.flowAssetsApplyFormListModel.workDate}];
    [self.expensePersonArr addObject:@{@"标段经理":self.flowAssetsApplyFormListModel.subProjectManager}];
    [self.expensePersonArr addObject:@{@"标段总监":self.flowAssetsApplyFormListModel.subProjectDirector}];
    [self.expensePersonArr addObject:@{@"标段成本核算管理部专员":self.flowAssetsApplyFormListModel.subProjectCostAccounting}];
    [self.expensePersonArr addObject:@{@"标段工管公司总经理":self.flowAssetsApplyFormListModel.subProjectGeneralManager}];
    if ([self.cellModel.processDefinitionKey isEqual:@"cshr_evection"]) {
        [self.expensePersonArr addObject:@{@"所属标段HR":self.flowAssetsApplyFormListModel.subPhaseHrName}];
    }
