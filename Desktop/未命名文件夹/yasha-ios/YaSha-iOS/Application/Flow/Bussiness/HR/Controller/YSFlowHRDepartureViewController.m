//
//  YSFlowHRDepartureViewController.m
//  YaSha-iOS
//
...
#import "YSFlowHRDepartureViewController.h"
#import "YSFlowHRInfoViewController.h"
#import "YSFlowAttachmentViewController.h"

@interface YSFlowHRDepartureViewController ()
...
@implementation YSFlowHRDepartureViewController
...
- (void)doNetworking {
    NSString *urlString;
   if([self.cellModel.processDefinitionKey isEqual:@"cshr_abnormal_turnover"]){
        urlString = [NSString stringWithFormat:@"%@%@/%@/%@",YSDomain, approveAbNormDismsApi, self.cellModel.businessKey,self.cellModel.processInstanceId];
    }else if([self.cellModel.processDefinitionKey isEqual:@"cshr_normal_turnover"]) {
        urlString = [NSString stringWithFormat:@"%@%@/%@/%@",YSDomain, approveNormDismsApi, self.cellModel.businessKey,self.cellModel.processInstanceId];
    }
...
- (void)setUpData {
    ...
    [self.expensePersonArr addObject:@{@"离职类型":self.flowAssetsApplyFormListModel.type}];
    [self.expensePersonArr addObject:@{@"离职日期":self.flowAssetsApplyFormListModel.leaveDate}];
    [self.expensePersonArr addObject:@{@"离职原因":self.flowAssetsApplyFormListModel.leaveReason}];
