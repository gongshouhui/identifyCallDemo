...
#import "YSYearSakaryLoginController.h"
...
@interface YSYearSakaryLoginController ()
...
@implementation YSYearSakaryLoginController

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    //修改左侧按钮图片
    [self.navigationController.navigationBar setBackgroundImage:[[UIImage alloc]init] forBarMetrics:(UIBarMetricsDefault)];
    [TalkingData trackPageBegin:@"年终奖"];
}

//设置统计离开该模块
- (void)viewWillDisappear:(BOOL)animated {
      [super viewWillDisappear:animated];
    [TalkingData trackPageEnd:@"年终奖"];
}
...
- (void)queryAction{
    ...
    [YSNetManager ys_request_GETWithUrlString:[NSString stringWithFormat:@"%@%@",YSDomain,getYearSalaryDetailApi] isNeedCache:NO parameters:dic successBlock:^(id response) {
        DLog(@"查询薪资:%@", response);
        [QMUITips hideAllToastInView:self.view animated:YES];
        if ([response[@"code"] integerValue] == 1) {
            YSSalaryDetailModel *model = [YSSalaryDetailModel yy_modelWithDictionary:response[@"data"]];
            YSYearSalaryDetailController *vc = [[YSYearSalaryDetailController alloc]init];
